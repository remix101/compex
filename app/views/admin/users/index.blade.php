        ...
        <div class="col-md-12">
            <div class="">
                <table id="table_users" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        <?php $ids = []; ?>
                        @foreach(User::desc('created_at')->get() as $i => $u)
                        @if($u->id != $user->id)
                        <tr {{ $u->status == Config::get('constants.USER_STATUS_BANNED') ? 'style="background-color:#ecc"' : '' }}>
                            <?php $ids[$i + 1] = $u->id ?>
                            <td>{{$i + 1}}</td>
                            <td>{{ $u->fullName }}</td>
